...
test.describe('Lazy Man 自动化内容生成与部署 v2', () => {
  test('完整流程：生成内容 → 等待完成 → 下载 → 推送 → 部署', async ({ page }) => {
    console.log('\n=== 自动化流程 v2：生成内容 → 下载 → 推送 → 部署 ===\n');
...
请详细展开每个部分，总字数约2000字。`;
      ...
      if (await sendButton.count() > 0) {
        ...
        // 截图生成的内容
        await page.screenshot({ path: 'test-results/lazyman-generated-content-v2.png', fullPage: true });
    ...
    if (await shareButton.count() > 0) {
      ...
      if (await textTab.count() > 0) {
        ...
        // 截图Text选项
        await page.screenshot({ path: 'test-results/lazyman-text-export-v2.png', fullPage: true });
        ...
        if (await downloadButton.count() > 0) {
          ...
          try {
            ...
            // 保存到项目目录
            const downloadPath = `${PROJECT_PATH}/content/docs/agent-platforms/coze/coze-workflow-practice-v2.mdx`;
...
slug: "coze-workflow-practice-v2"
...
`;
            ...
            try {
              ...
              execSync('cd ~/aidz-fun && git add content/docs/agent-platforms/coze/coze-workflow-practice-v2.mdx');
              console.log('  ✅ 文件已添加到Git');

              // 提交
              console.log('  提交更改...');
              const commitMessage = 'feat: 添加Coze工作流实战指南文章(v2)';
              ...
              try {
                ...
                console.log('\n  访问地址: https://aidz.fun/docs/agent-platforms/coze/coze-workflow-practice-v2');
