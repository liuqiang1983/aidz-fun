...
test.describe('Lazy Man 自动化内容生成与部署', () => {
  test('完整流程：生成内容 → 下载 → 推送 → 部署', async ({ page }) => {
    console.log('\n=== 自动化流程：生成内容 → 下载 → 推送 → 部署 ===\n');
      ...
      if (await textTab.count() > 0) {
        ...
        if (await downloadButton.count() > 0) {
          ...
          try {
            ...
            console.log(`  📝 文件前100字符: ${fileContent.substring(0, 100)}...`);
...
`;
            ...
            try {
              ...
              // 提交
              console.log('  提交更改...');
              const commitMessage = 'feat: 添加Coze工作流实战指南文章';
              ...
              try {
                ...
                console.log('\n  访问地址: https://aidz.fun/docs/agent-platforms/coze/coze-workflow-practice');
            ...
            if (await copyButton.count() > 0) {
              console.log('  尝试使用Copy按钮...');
              await copyButton.click();
              console.log('  ✅ 内容已复制到剪贴板');
              
              // 提示用户手动保存
              console.log('  ⚠️ 请手动粘贴内容到文件');
            }
          }
        } else {
