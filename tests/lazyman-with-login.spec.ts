...
const LOGIN_EMAIL = '363845354@qq.com';
const LOGIN_PASSWORD = 'liuqiang1@';

test.describe('Lazy Man 智能自动化部署（含登录）', () => {
  test('完整流程：登录 → 生成内容 → 智能等待 → 下载 → 推送', async ({ page }) => {
    test.setTimeout(700000); // 设置测试超时为700秒
    console.log('\n=== 智能自动化流程：登录 → 生成内容 → 下载 → 推送 ===\n');
    ...
    // 步骤2: 检查是否已登录，如果未登录则执行登录
    console.log('\n📍 步骤2: 检查登录状态');

    // 检查是否有登录按钮或用户头像
    const loginButton = page.locator('button:has-text("登录"), button:has-text("Login"), a:has-text("登录"), a:has-text("Login")').first();
    const userAvatar = page.locator('[class*="avatar"], [class*="user"], [class*="profile"]').first();

    const hasLoginButton = await loginButton.count() > 0;
    const hasUserAvatar = await userAvatar.count() > 0;

    console.log(`  📊 登录按钮: ${hasLoginButton}, 用户头像: ${hasUserAvatar}`);

    if (hasLoginButton && !hasUserAvatar) {
      ...
      console.log('  📸 已保存登录界面截图');

      // 查找邮箱输入框
      const emailInput = page.locator('input[type="email"], input[placeholder*="邮箱"], input[placeholder*="email"], input[name*="email"]').first();
      const passwordInput = page.locator('input[type="password"], input[placeholder*="密码"], input[placeholder*="password"], input[name*="password"]').first();
...
请详细展开每个部分，总字数约2000字。`;
      ...
      // 截图填充后的状态
      await page.screenshot({ path: 'test-results/05-filled.png', fullPage: true });
      console.log('  📸 已保存填充后截图');

      // 按Enter键发送
      console.log('  🖱️ 按Enter键发送...');
      await messageInput.press('Enter');
      await page.waitForTimeout(2000);

      // 截图发送后的状态
      await page.screenshot({ path: 'test-results/06-after-send.png', fullPage: true });
      ...
      // 截图最终状态
      await page.screenshot({ path: 'test-results/07-generated-content.png', fullPage: true });
      ...
      if (shareButton) {
        ...
        // 截图分享界面
        await page.screenshot({ path: 'test-results/08-share-modal.png', fullPage: true });
        ...
        if (await textTab.count() > 0) {
          ...
          // 截图Text选项
          await page.screenshot({ path: 'test-results/09-text-export.png', fullPage: true });
          ...
          if (await downloadButton.count() > 0) {
            ...
            try {
              ...
              if (fileContent.length > 100) {
                ...
                // 添加frontmatter
                console.log('\n📍 步骤8: 添加Contentlayer frontmatter');
                const frontmatter = `---
title: "Coze工作流实战指南"
