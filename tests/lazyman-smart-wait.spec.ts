...
test.describe('Lazy Man 智能自动化部署', () => {
  test('智能等待：动态检测内容生成完成', async ({ page }) => {
    ...
    await page.goto(LAZYMAN_URL, { waitUntil: 'networkidle' });
    ...
    if (!messageInput) {
      console.log('  ❌ 未找到输入框，尝试查找所有textarea和contenteditable');
      const allTextarea = await page.locator('textarea').all();
      const allEditable = await page.locator('[contenteditable="true"]').all();
      console.log(`  📝 找到textarea: ${allTextarea.length}个`);
      console.log(`  📝 找到contenteditable: ${allEditable.length}个`);

      if (allTextarea.length > 0) {
        messageInput = allTextarea[0];
      } else if (allEditable.length > 0) {
        messageInput = allEditable[0];
      }
    }
...
请详细展开每个部分，总字数约2000字。`;
      ...
      if (!sendButton) {
        console.log('  ⚠️ 未找到明确的发送按钮，尝试查找所有可见按钮');
        const allButtons = await page.locator('button:visible').all();
        console.log(`  🔘 找到可见按钮: ${allButtons.length}个`);

        // 查找最后一个可见按钮（通常是发送按钮）
        if (allButtons.length > 0) {
          sendButton = allButtons[allButtons.length - 1];
          const btnText = await sendButton.innerText();
          console.log(`  🔘 使用最后一个按钮: "${btnText}"`);
        }
      }
      ...
      if (sendButton) {
        ...
        while (Date.now() - startTime < maxWaitTime) {
          ...
          // 检测3: 检查内容长度变化
          const contentArea = page.locator('[class*="message"], [class*="content"], [class*="response"]').first();
          let currentContentLength = 0;
          if (await contentArea.count() > 0) {
            const contentText = await contentArea.innerText();
            currentContentLength = contentText.length;
          }
          ...
          const timeSinceLastChange = Math.floor((Date.now() - lastChangeTime) / 1000);

          console.log(`  📊 [${elapsedSeconds}s] 停止按钮=${hasStopButton}, 加载中=${hasLoading}, 内容长度=${currentContentLength}, 页面长度=${bodyLength}, 稳定=${stableCount}, 距上次变化=${timeSinceLastChange}s`);
